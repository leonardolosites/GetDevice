<?php
	...
	$filter = " instituicao_id_instituicao = {$id_instituicao} AND ((data_agendamento >= '{$dataAtual}' AND horario_final_agendamento > '{$horaAtual}') OR (data_agendamento > '{$dataAtual}')) ";

	if($userType == 1){
		$sqlListaAgd = "SELECT * FROM agendamento WHERE {$filter} ORDER BY data_agendamento DESC";
	}else{
		$sqlListaAgd = "SELECT * FROM agendamento WHERE usuario_id_usuario = {$userId} AND {$filter} ORDER BY data_agendamento ASC";
	}
			...
			foreach ($agendamentos as $agd) {
				...
				$lista .= "
					<tr class='{$status}'>
						...
						<td><button type='button' onclick='cancelaAgd({$agd['id_agendamento']}, {$id_instituicao})' class='btn btn-sm btn-danger w-100'>Cancelar</button></td>
					</tr>
				";
			}		
		}else{
